...
public class MyWebSocketDecoder extends ReplayingDecoder<MyWebSocketDecoder.State> implements WebSocketFrameDecoder {

	private static final InternalLogger logger = InternalLoggerFactory.getInstance(MyWebSocketDecoder.class);
	 ...
	 * Constructor
	 * 
	 * @param maskedPayload Web socket servers must set this to true processed incoming masked payload. Client
	 *            implementations must set this to false.
	 * @param allowExtensions Flag to allow reserved extension bits to be used or not
	 * @param maxFramePayloadLength Maximum length of a frame's payload. Setting this to an appropriate value for you
	 *            application helps check for denial of services attacks.
	 */
	public MyWebSocketDecoder(boolean maskedPayload, boolean allowExtensions, int maxFramePayloadLength) {
		...
		try {
			...
			case PAYLOAD:
				...
				if (frameOpcode == OPCODE_TEXT) {
					...
					TextWebSocketFrame textWebSocketFrame =  new TextWebSocketFrame(frameFinalFlag, frameRsv, framePayload);
					out.add(textWebSocketFrame);
					
					System.out.println("客户端发来的数据："+textWebSocketFrame.text());
